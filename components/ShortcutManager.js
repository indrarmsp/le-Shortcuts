                ...
                <div className="link-hover-actions" style={{
                    ...
                    <div style={{ display: 'flex', gap: '2px' }}>
                        <button {...attributes} {...listeners} className="btn-icon drag-handle"
                            style={{ width: '26px', height: '26px', cursor: selectMode ? 'pointer' : 'grab', touchAction: 'none', opacity: selectMode ? 0.9 : undefined }}
...
function SortableCategorySection({ cat, catLinks, openCat, openLink, deleteCategory, deleteLink, toggleCollapsed, collapsed, recordClick, togglePin, toggleCategoryPin, reorderLinks, selectMode, selectedLinks, onToggleSelectLink, categorySelectMode, selectedCategories, onToggleSelectCategory }) {
        ...
        <section ref={setNodeRef} style={style} className="card" key={cat.id} data-cat-id={cat.id}>
            <div style={{ padding: '1.25rem 1.5rem 0 1.5rem' }}>
                <div style={{ display: 'flex', alignItems: 'center', gap: '10px', marginBottom: collapsed ? '1.25rem' : '1rem' }}>
                    ...
                    {categorySelectMode && (
                        <button className="btn-icon" onClick={() => onToggleSelectCategory(cat.id)} title="Select category" style={{ width: '28px', height: '28px', flexShrink: 0 }}>
                    ...
                    <div style={{ marginLeft: 'auto', display: 'flex', gap: '2px' }}>
                        <button className="btn-icon" onClick={() => toggleCategoryPin(cat.id)} title={cat.pinned ? 'Unpin category' : 'Pin category'}>
                            {cat.pinned ? <PinOff size={15} style={{ color: 'var(--accent-text)' }} /> : <Pin size={15} />}
                        </button>
...
export default function ShortcutManager() {
    const {
        ...
        recordClick, togglePin, toggleCategoryPin, reorderLinks, toggleCollapsed
    } = useDashboardData();
    ...
    useEffect(() => {
        const onKey = (e) => {
            ...
            if (e.key === 'Escape') {
                ...
                if (categorySelectMode) { setCategorySelectMode(false); setSelectedCategories(new Set()); }
                else setModal(null);
            }
    ...
    }, [categories, categorySelectMode, selectMode]);
        ...
        <>
            <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: '1.5rem', gap: '1rem', flexWrap: 'wrap' }}>
                ...
                <div style={{ display: 'flex', gap: '0.6rem', alignItems: 'center' }}>
                    ...
                    <button
                        onClick={() => {
                            const next = !(selectMode || categorySelectMode);
                            setSelectMode(next);
                        ...
                        className={selectMode || categorySelectMode ? 'btn-primary' : 'btn-ghost'}
                ...
                <div className="card" style={{
                    ...
                    <div style={{ marginLeft: 'auto', display: 'flex', gap: '0.5rem' }}>
                        <button className="btn-ghost" style={{
                            ...
                            <Trash2 size={14} /> Delete Categories
                        </button>
                        <button className="btn-icon" onClick={() => { setCategorySelectMode(false); setSelectedCategories(new Set()); }}
                        ...
                        {categories.map(cat => {
                            ...
                            return (
                                <SortableCategorySection
                                    ...
                                    togglePin={togglePin}
                                    toggleCategoryPin={toggleCategoryPin}
